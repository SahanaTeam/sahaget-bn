...
def validate_name(first_name, last_name):
    ...
    if not last_name:
        return Response(
            {"error": "Last name can not be blank."}, status=status.HTTP_400_BAD_REQUEST
        )
...
def validate_password_field(password, confirm_password):
    if not password:
        return Response(
            {"error": "Password cannot be blank."}, status=status.HTTP_400_BAD_REQUEST
        )
...
def validate_username_field(username):
    if not username:
        return Response(
            {"error": "Username cannot be blank."}, status=status.HTTP_400_BAD_REQUEST
        )
...
def validate_user_registration_data(data):
    ...
    username_error = validate_username_field(username)
    if username_error:
        return username_error

    name_error = validate_name(first_name, last_name)
    if name_error:
        return name_error

    password_error = validate_password_field(password, confirm_password)
    if password_error:
